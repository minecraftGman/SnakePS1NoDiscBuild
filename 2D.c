...
void sprite_create(unsigned char imageData[], int x, int y, GsSPRITE **sprite) {
	...
	*sprite = malloc3(sizeof(GsSPRITE));
	(*sprite) -> attribute = (tim_data->pmode & 3) << 24;
	(*sprite) -> x = x;
	(*sprite) -> y = y;

	switch (tim_data->pmode & 3) {		// X-Value depends on BitDepht

    case 0: (*sprite)->w = tim_data->pw << 2;
            (*sprite)->u = (tim_data->px & 0x3f) * 4;
            break;
    case 1: (*sprite)->w = tim_data->pw << 1;
            (*sprite)->u = (tim_data->px & 0x3f) * 2;
            break;
    default: (*sprite)->w = tim_data->pw;
             (*sprite)->u = tim_data->px & 0x3f; 
    };
